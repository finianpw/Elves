...
## Wersja Android (nowa)
Dodano osobny projekt `ElfForestSaga.Android` (C# `net8.0-android`) z flow:
1. ekran menu startowego,
2. start gry po kliknięciu `Start Gry`,
3. powrót do menu przyciskiem systemowym `Back`.

### Sterowanie na telefonie
- lewa strona: **wirtualna gałka** (ruch + skok ruchem gałki do góry),
- prawa strona: **przycisk STRZAŁ**.

### Uruchamianie Android
1. Zainstaluj .NET 8 + Android workload (`dotnet workload install android`) oraz Android SDK.
2. Podłącz urządzenie/emulator Android.
3. Uruchom:
   ```bash
   dotnet build ElfForestSaga.Android/ElfForestSaga.Android.csproj -t:Run
   ```
