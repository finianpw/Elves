...
public sealed class GameForm : Form
{
    ...
    [
        "Aerin, elf o rysach inspirowanych bohaterami fantasy, walczy o Serce Lasu.",
        "Wrogowie mają animowane sylwetki i aktywnie atakują podczas pościgu.",
    ...
    private void UpdateEnemies(Level level)
    {
        foreach (var enemy in level.Enemies)
        {
            ...
            var closeEnoughToChase = Math.Abs(playerDistance) < 300;
            ...
            enemy.AnimationPhase += 0.2f + (closeEnoughToChase ? 0.08f : 0f);
            ...
            if (enemy.Type == EnemyType.ElderWizard && closeEnoughToChase && enemy.AttackCooldown == 0)
            {
                var vx = directionToPlayer * 5f;
                _enemyProjectiles.Add(new EnemyProjectile
                {
                    Bounds = new RectangleF(enemy.Bounds.X + 18, enemy.Bounds.Y + 16, 10, 10),
                    VelocityX = vx,
                    VelocityY = 0,
    ...
    private void DrawBackground(Graphics g)
    {
        ...
        g.FillEllipse(new SolidBrush(Color.FromArgb(190, 236, 236, 210)), ClientSize.Width - 220, moonY, 112, 112);
    ...
    private void DrawPlatforms(Graphics g)
    {
        foreach (var platform in CurrentLevel.Platforms)
        {
            ...
            g.DrawRectangle(new Pen(Color.FromArgb(44, 44, 44)), platform.Bounds.X, platform.Bounds.Y, platform.Bounds.Width, platform.Bounds.Height);
            ...
            g.FillRectangle(new SolidBrush(Color.FromArgb(31, 91, 47)), platform.Bounds.X, platform.Bounds.Y - 7, platform.Bounds.Width, 7);
        }
    }
    ...
    private void DrawDecor(Graphics g)
    {
        for (var i = 0; i < 24; i++)
        {
            var x = i * 155 + 100;
            var y = 610 + (i % 2) * 8;
            g.FillEllipse(new SolidBrush(Color.FromArgb(45, 129, 77)), x, y, 17, 11);
            g.FillEllipse(new SolidBrush(Color.FromArgb(82, 58, 20)), x + 6, y + 6, 4, 10);
        }
    }
    ...
    private void DrawEnemies(Graphics g)
    {
        ...
        {
            var bodyBrush = enemy.Type switch
            {
            ...
            // tułów
            g.FillRoundedRectangle(bodyBrush, enemy.Bounds, 8);
            // głowa
            g.FillEllipse(bodyBrush, enemy.Bounds.X + 10, enemy.Bounds.Y - 10, 24, 22);
            // nogi
            g.DrawLine(new Pen(Color.FromArgb(38, 30, 26), 4), enemy.Bounds.X + 14, enemy.Bounds.Bottom - 2, enemy.Bounds.X + 12 + limbSwing, enemy.Bounds.Bottom + 12);
            g.DrawLine(new Pen(Color.FromArgb(38, 30, 26), 4), enemy.Bounds.Right - 14, enemy.Bounds.Bottom - 2, enemy.Bounds.Right - 12 - limbSwing, enemy.Bounds.Bottom + 12);
            // ręce
            g.DrawLine(new Pen(Color.FromArgb(38, 30, 26), 4), enemy.Bounds.X + 5, enemy.Bounds.Y + 20, enemy.Bounds.X - 8, enemy.Bounds.Y + 28 + armRaise);
            g.DrawLine(new Pen(Color.FromArgb(38, 30, 26), 4), enemy.Bounds.Right - 5, enemy.Bounds.Y + 20, enemy.Bounds.Right + 8, enemy.Bounds.Y + 28 + armRaise);
    ...
    private void DrawHud(Graphics g)
    {
        ...
        g.DrawString($"Broń: {(_player.SelectedWeapon == WeaponType.Bow ? "Łuk" : "Ogień")} (Z)", _hudFont, Brushes.White, 18, 57);
        g.DrawString($"Jasność: {(int)(_brightness * 100)}%", _hudFont, Brushes.White, 18, 79);
        g.DrawString("ESC - menu (poziom, jasność, restart, wyjście)", _hudFont, Brushes.White, 300, 79);

        for (var i = 0; i < _storyParagraphs.Length; i++)
        {
            g.DrawString(_storyParagraphs[i], _storyFont, Brushes.LightCyan, 300, 12 + (i * 20));
        }
    }
